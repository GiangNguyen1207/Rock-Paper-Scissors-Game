...
function checkWinner() {
  ...
  const isCont = winners.map(
    (winner, index, arr) =>
      winner !== "Tie" && winner === arr[index - 1] && winner === arr[index - 2]
  );
  ...
  for (const point in points) {
    if (points[point] === 10) {
      result.textContent = `END GAME!!! ${point.toUpperCase()} IS THE FINAL WINNER`;
      for (const player of players) {
        player.textContent.toLowerCase() === point
          ? (player.textContent = "WINNER")
          : null;
      }
      for (const group of groups) {
        group.classList.value === point ? group.classList.add("winner") : null;
      }
      isGamePlaying = false;
    }
  }
  ...
  if (index > 0) {
    result.textContent = `END GAME!!! ${winners[
      index
    ].toUpperCase()} IS THE FINAL WINNER`;
    for (const player of players) {
      player.textContent.toLowerCase() === winners[index]
        ? (player.textContent = "WINNER")
        : null;
    }
    for (const group of groups) {
      group.classList.value === winners[index]
